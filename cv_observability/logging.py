...
_PHONE_IN_TEXT_RE = re.compile(r"(?<!\w)\+\d{10,15}(?!\d)")
...
def _sanitize_text(value: str, *, limit: int = 1200) -> str:
    ...
    text = _PHONE_IN_TEXT_RE.sub("[REDACTED_NUMBER]", text)
    return text[:limit]
...
class EventLogger:
    """Structured logger optimized for GitHub Actions and local CLI runs.

    Every line is human-scannable and machine-parseable. When `event_log_path`
    (or CVFIT_EVENT_LOG) is set, the same event is appended as JSONL. Secret
    values and common PII are redacted, while observability metrics such as
    prompt_tokens and total_tokens remain visible.
    """
