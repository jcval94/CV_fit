...
</style></head><body><main><header><h1>Today's CV_fit vacancies</h1><p>Search date <strong>{html.escape(target_date)}</strong>. Source fit and deterministic evidence coverage are intentionally separate. A CV is marked ready only after content quality, cover letter, design review and Chromium/PDF physical-layout gates pass.</p></header><section class="grid">{''.join(cards)}</section><div class="note">Public preview uses only the repository's public-safe candidate identity. Email and phone are never published here.</div></main></body></html>"""
